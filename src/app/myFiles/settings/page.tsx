...
function Page() {
  ...
  const pageElemRender = pageElem.map(({ data, category }, idx) => (
    <div key={idx} className="flex flex-col mt-5 gap-5">
      <h1 className="font-bold">{t(category)}</h1>
      {data.map(({ title, details, outher }, idx) => (
        <div key={idx} className="flex flex-col">
          ...
          <p className="text-gray-500 flex flex-wrap md:relative lg:relative xl:relative gap-2">
            {t(details)}
            <span
              className={`md:absolute lg:absolute xl:absoluteF ${i18n.language == "ar" ? "left-0" : "right-0"} text-black`}
            >
              {t(outher)}
            </span>
          </p>
      ...
      {!showEdit ? (
        ...
        <div>
          <div className="flex">
            ...
            <IoIosCloseCircle
              onClick={handleClick}
              className={`text-3xl ${i18n.language == "ar" ? "mr-40" : "ml-40"}`}
            />
          </div>
          <p className="text-gray-500 mt-3 mb-8">
            {t("Manage your account settings snd prefernces")}
          </p>
          <div>
